...
func (this *MachineSpec) EqualVT(that *MachineSpec) bool {
	...
	if this.HaRegistered != that.HaRegistered {
		return false
	}
...
func (m *MachineSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 13:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field HaRegistered", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
